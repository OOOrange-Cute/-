...
class VolumeClassifier(object):
    ...
    def __init__(self,
        ...
        self.feature_extractor = nn.Sequential(*list(self.net.children())[:-1])  # 去掉最后的全连接层"""
    ...
    def trainer(self,
        ...
        """train_transformer = transforms.Compose([
            tr.ToCVImage(),  # 转换为3通道图像
            tr.RandomResizedCrop(size=224, scale=(0.8, 1.0), ratio=(0.75, 1.33)),  # 随机裁剪并调整大小
            tr.RandomHorizontalFlip(p=0.5),  # 随机水平翻转
            tr.ToTensor(),  # 转换为PyTorch张量
        ...
        train_transformer = transforms.Compose([
            tr.ToCVImage(),  # 转换为3通道图像
            tr.RandomResizedCrop(size=224, scale=(0.5, 1), ratio=(0.75, 1.33)),  # 随机裁剪并调整大小
            tr.RandomHorizontalFlip(p=0.5),  # 随机水平翻转
            tr.RandomRotation(degrees=15),  # 随机旋转
            tr.RandomBrightnessContrast(brightness=0.2, contrast=0.2),  # 随机亮度/对比度调整
            tr.RandomGaussianBlur(kernel_size=5, sigma=1.5),  # 随机高斯模糊
            tr.ToTensor(),  # 转换为PyTorch张量
            tr.Normalize(self.train_mean, self.train_std)  # 标准化
        ])

        val_transformer = transforms.Compose([
            tr.ToCVImage(),  # 转换为3通道图像
            tr.Resize(resized=224),  # 调整大小
            tr.ToTensor(),  # 转换为PyTorch张量
            tr.Normalize(self.train_mean, self.train_std)  # 标准化
        ])
        ...
        self._plot_accuracy(train_accs, val_accs, output_dir)
        self._plot_loss(train_losses, val_losses, output_dir)
        
    def _plot_accuracy(self, train_accs, val_accs, output_dir):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.title('Batch Normalization Training and Validation Accuracy')
        plt.legend()
        plt.savefig(os.path.join(output_dir, 'accuracy_curve.png'))
        plt.show()
        
    def _plot_loss(self, train_losses, val_losses, output_dir):
        import matplotlib.pyplot as plt
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Losses')
        plt.title('Batch Normalization Training and Validation Losses')
        plt.legend()
        ...
        for step, sample in enumerate(train_loader):
            ...
            """for name, module in net.named_modules():
                if isinstance(module, BatchNorm2d):
                    self.writer.add_scalar(f'{name}/running_mean', module.running_mean.mean(), self.global_step)
                    self.writer.add_scalar(f'{name}/running_var', module.running_var.mean(), self.global_step)"""
        ...
        if net_name.startswith('res') or net_name.startswith('wide_res'):
           ...
           for m in net.modules():
               if isinstance(m, BatchNorm2d):
                   m.momentum = 0.1  # 设置批量归一化的 momentum 参数
                   m.eps = 1e-5  # 设置批量归一化的 eps 参数
...
def supervised_contrastive_loss(features, labels, temperature=0.1):
    """
    计算监督对比损失
    :param features: 模型输出的特征向量 (N, D)
    :param labels: 样本的真实标签 (N,)
    :param temperature: 温度参数
    :return: 对比损失
    """
    # 计算特征向量的相似度矩阵
    similarity_matrix = torch.matmul(features, features.T) / temperature
    # 排除对角线上的相似度（即自身与自身的相似度）
    mask = torch.eye(similarity_matrix.size(0), dtype=torch.bool).to(similarity_matrix.device)
    similarity_matrix = similarity_matrix.masked_fill(mask, -1e9)
    
    # 计算正样本和负样本的相似度
    positive_samples = similarity_matrix[labels[:, None] == labels[None, :]]
    negative_samples = similarity_matrix[labels[:, None] != labels[None, :]]
    
    # 计算对比损失
    positive_loss = -torch.log(positive_samples / (positive_samples + negative_samples.sum(dim=1, keepdim=True)))
    negative_loss = -torch.log(negative_samples / (positive_samples + negative_samples.sum(dim=1, keepdim=True)))
