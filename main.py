...
if __name__ == "__main__":
    ...
    parser.add_argument('-m',
                        '--mode',
                        default='train',
                        choices=["train-cross","train", "inf","inf-cross","inf-ensemble"],
                        help='choose the mode',
                        type=str)
    parser.add_argument('--ensemble',
                        action='store_true', 
                        help='perform ensemble inference')
    ...
    ###############################################
            ...
            for fold in range(1,FOLD_NUM+1):
                ...
                # 开始推理
                result, _, _ = classifier.inference(test_path, label_dict, hook_fn_forward=False)
            ...
            true_labels = np.array(list(label_dict.values()))
            ensemble_acc = np.mean(ensemble_pred == true_labels)
            print(f'Ensemble Model Accuracy: {ensemble_acc:.4f}')
            
            
            # 保存集成结果
            save_dir = './analysis/result/ensemble'
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            
            # 保存预测结果
            result = {'path': test_path, 'true': label_dict.values(), 'pred': ensemble_pred, 'prob': all_probs}
            csv_file = pd.DataFrame(result)
            csv_file.to_csv(os.path.join(save_dir, 'ensemble_result.csv'), index=False)
            
            # 生成分类报告
            cls_report = classification_report(result['true'], result['pred'], output_dict=True)
            report_csv_file = pd.DataFrame(cls_report)
            report_csv_file.to_csv(os.path.join(save_dir, 'ensemble_report.csv'))